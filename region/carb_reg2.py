...
def utility_function(ranking, x_t_list, tho_list, x):  # ranking[i] < ranking[j] => on prefere i à j.
    ...
    i = 0
    for xi in x:
        sum += (1 / ranking[i]) * simple_utility_function(x_t_list[i], tho_list[i], xi)
        i += 1
    return sum

def u(x):
    return utility_function([1,2,3], [1,1,1], [0.5, 0.5, 0.5], x)

print(u([0,0,0]))
N = 1000

iledefrance = region("Ile de France", u, 12300000/N, 0)
auvergne = region("Auvergne", u, 8200000/N, 1)
provence = region("Provence", u, 5160000/N,2)
bretagne = region("Bretagne", u, 3400000/N, 3)
paysdeloire = region("Pays de la Loire",u ,3900000/N,4)
corse = region("Corse", u, 351000/N,5)
